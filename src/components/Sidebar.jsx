export default function Sidebar({ page, setPage, collapsed, setCollapsed, user }) {
  ...
  const canAddCase = user?.role === 'Lawyer' || user?.id === 'lawyer'
  ...
  const mainItems = isAdmin 
    ...
    : [
        ...
        { id: 'prioritization', icon: 'fa-robot', label: 'AI Prioritization' },
        ...(canAddCase ? [
            { id: 'newcase', icon: 'fa-plus-circle', label: 'File New Case' },
            { id: 'schedule', icon: 'fa-calendar-alt', label: 'My Schedule' }
        ] : []),
        { id: 'ai-analysis', icon: 'fa-magic', label: 'AI Analysis' }
      ]
